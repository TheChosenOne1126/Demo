    ...
    : CommandList(new FUICommandList)
{
    ...
    CommandList->MapAction(FUnLuaEditorCommands::Get().ExportGameplayTags, FExecuteAction::CreateLambda([]() -> void
    {
    	FGameplayTagContainer AllTags;
    	UGameplayTagsManager::Get().RequestAllGameplayTags(AllTags, true);
    	
    	FString LuaContent = TEXT("--- Auto-generated GameplayTags ---\n");
    	LuaContent += TEXT("--- Generated by UnLua Editor ---\n");
    	LuaContent += TEXT("local RequestGameplayTag = UE.UStatics.RequestGameplayTag\n");
    	LuaContent += TEXT("local GameplayTags = {\n");
    	
    	const TArray<FGameplayTag>& TagList = AllTags.GetGameplayTagArray();
    	for (int i = 0; i < TagList.Num(); ++i)
    	{
    		FString SafeName = TagList[i].ToString().Replace(TEXT("."), TEXT("_"));
    		SafeName = SafeName.Replace(TEXT("("), TEXT("_"));
    		SafeName = SafeName.Replace(TEXT(")"), TEXT(""));
    		LuaContent += FString::Printf(TEXT("    %s = RequestGameplayTag(\"%s\")"), *SafeName, *TagList[i].ToString());
...
TSharedRef<SWidget> FMainMenuToolbar::GenerateUnLuaSettingsMenu()
{
    ...
    MenuBuilder.AddMenuEntry(Commands.ExportGameplayTags, NAME_None, LOCTEXT("ExportGameplayTags", "Export GameplayTags"));
